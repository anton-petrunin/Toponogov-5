...
If $\tilde x_1$ has a single edge $(\tilde x_1,\tilde x_2)$, then 
\begin{align*}
...
adjacent points.
If not, then $\tilde x_i$ and all its adjacent points lie in a finite-dimensional half-space with boundary plane $\Pi$.
...
By these conditions, $\tilde q_1$ must lie on the circle with center at $x_1$ and radius $|x_1-q|_F$.
...
Once this case is excluded, by the key lemma, we can assume that two tense triples in $F$ have a common center.
...
First, we construct an Alexandrov space $L$ --- a flat disc with at most four singular points.
...
\parit{Step 1.}
In this step, we describe three groups of conditions on these 12 angles and show that together they guarantee that $L$ has nonnegative curvature in the sense of Alexandrov and $\iota$ is distance-noncontracting.
...
%Moreover, it implies that any minimizing geodesic in $L$ may cross an edge $[\hat x,\hat v_i]$ or $[\hat x,\hat w_j]$ at most once.
...
Let us show that these conditions imply that $\iota$ distance-noncontracting.
Suppose $\gamma$ is a curve from $\hat x$ to $\hat v_i$ that lies completely in one of triangles adjacent to the edge $\hat x \hat v_i$.
Note that the inequalities in $({*})$ imply that 
\[\length \gamma\ge | x -\hat v_i|_F.\]
Further, the same holds for any pair $(\hat x,\hat w_i)$ and $(\hat v_i,\hat w_j)$.
...
for each $i$.
We also get that each of four edges $\hat x \hat v_i$ or $\hat x \hat w_i$ is a convex set in $L$;
...
If points $z_1$ and $z_2$ lie on the left side from $\gamma$,
...
If points $z_1$ and $z_2$ lie on the right side from $\gamma$,
then we arrive at a contradiction with
\[
...
Note that in addition we get that two edges $\hat v_1\hat x$ and $\hat x\hat v_2$ form a shortest path in $L$; the same holds for $\hat w_1\hat x$ and $\hat x\hat w_2$.
...
Recall that $\gamma$ crosses each of four edges $\hat x \hat v_i$ or $\hat x \hat w_i$ at most once.
Therefore $\gamma$ have to cross edge $\hat x \hat v_2$.
