...
\hypersetup{pdftitle={5-point Toponogov theorem},
...
If all we can choose all positive $\lambda_i$ in $({*})$, then we say that $(p,x_1,\dots,x_n)$ is a \emph{nondegenerate tense array}.
...
Otherwise, if $\tilde p$ lies on a side, say $[\tilde x,\tilde y]$ of nondegenerate triangle $[\tilde x\tilde y\tilde z]$, then $(\tilde p,\tilde x,\tilde y,\tilde z)$ is degenerate.
...
This set defines a subspace of quadratic forms of codimension 2 in the 10-dimesnional space of quadratic forms on $\RR^4$.
Since $k\le 4$, 
taking intersection of all such subspaces we get a subspace $S$ of dimension at least $2$.
...
Choose a positive quadratic form $r$ and minimal $t>0$ such that  
\[W_{\bm{x}}(w)+s(w)+t\cdot r(w)\ge 0
\leqno({*}{*})\] 
for any $w\in K_5$.
Note that for some $w\in K_5$, we have equality in $({*}{*})$.
 
The metric on $\bm{x}$ that corresponds to the form $W_{\bm{x}}+s+t\cdot r$ satisfies all $\LSS(4)$ inequalities and
by \ref{clm:nondeg-tense}, it has a tense array.

Consider a limit array $l$ of such arrays as $s\to 0$. 
Note that $l$ is isometric to an array in the Euclidean space and it cannot be one of $k$ tense array, for the original metric.
Since $W_{\bm{x}}+s+t\cdot r\to W_{\bm{x}}$ as $s\to 0$, the array $l$ must contain one of $k$ arrays for the original metric.
That is one of the three-point array sits in a four point array that is isometric to an array in the Euclidean space.
...
\begin{itemize}
\item $F$ contains a 5-point tense set in this case $F$ admits an isometric embedding into Euclidean space.
...
By the definition of tense array, we can choose an array $(\tilde p, \tilde x_1, \tilde x_2, \tilde x_3)$ in $\RR^2$ that is isometric to  $(p, x_1, x_2, x_3)$.
...
We can assume that one of ellipsoids is not degenerate; otherwise, the array $\tilde p$, $\tilde q$, $\tilde x_1$, $\tilde x_2$, $\tilde x_3$ is isometric to $F$;
...
Other triple of ellipsoids, say $P$, $X_1$, $X_2$ might be degenerate only if $\tilde p\in[\tilde x_1,\tilde x_2]$.
...
Now, suppose exactly two ellipsoins, note that it cannot be $P$.
...
\parit{Final step.} It remains to modify $L$ into a plane with a smooth Riemannian metric.
...
By \ref{clm:10-2k}, the space of quadratic forms on $\RR^4$ contains a subspace $S$ in the of dimesnion at least 2 such that for any form $U\in S$ for all $t$ sufficiently close to 0, the forms $W+t\cdot U$ satisfy all $\LSS(4)$.
In order to be on an extremal ray, the space $S$ must be one-dimensional --- a contradiction.
