...
where $a_{ij}=|p-x_i|_X^2+|p-x_j|_X^2-|x_i-x_j|_A^2$
...
We say that an array $(p,x_1,\dots,x_n)$ is \emph{tense} with marked point $p$ if configuration described by the $(n+1)$-comparison with center at $p$ is unique up to congruence and isometric to the original array.
Note that if $w\in K_{n+1}$ is ..., 
